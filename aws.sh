...
##############Determine IP addresses, status of instanses and ID of stopped EC2 instances#######################
...
#aws ec2 describe-instances  --filter "Name=ip-address,Values=$address" --query "Reservations[].Instances[][PublicIpAddress, State.Name, InstanceId]" 
echo -e "State:"
...
#	echo $DEL
	...
	aws ec2 describe-images --owners 717986625066  --filter "Name=name,Values=*"$DEL"*" | grep "ImageId" | awk '{print $2}'|cut -d '"' -f 2 >>/tmp/ami_id


		for ami in `cat /tmp/ami_id`
		do
		aws ec2 describe-images --owners 717986625066  --image-ids $ami | grep snap | awk ' { print $2 }' >> /tmp/snapshots.txt
